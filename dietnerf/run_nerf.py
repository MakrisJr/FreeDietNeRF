...
def train():
    ...
    wandb.init(project=args.wandb_project, entity=args.wandb_entity) # Initialize wandb
    wandb.run.name = args.expname # Set the run name
    wandb.run.save() # Save the run id
    wandb.config.update(args) # Save the arguments
    ...
    if args.dataset_type == 'blender':
        ...
        if args.white_bkgd:
            images = images[...,:3]*images[...,-1:] + (1.-images[...,-1:]) # apply transparency to image + inverse of alpha channel
        else:
            images = images[...,:3] # ignore alpha channel
    else:
